...
window.editLimit = async function(id) {
    try {
        const doc = await db.collection('document_limits').doc(id).get();
        if (!doc.exists) return;
        const data = doc.data();

        document.getElementById('limit-id').value = id;
        document.getElementById('limit-doc').value = data.documentName || '';
        document.getElementById('limit-action').value = data.actionName || '';
        document.getElementById('limit-max').value = data.maxLimit || '';
        document.getElementById('limit-used').value = data.usedCount || 0;
        document.getElementById('limit-notes').value = data.notes || '';

        const modal = bootstrap.Modal.getOrCreateInstance(document.getElementById('addLimitModal'));
        modal.show();
    } catch (e) {
        console.error('Error editing limit tracker:', e);
        if (window.dashboard) window.dashboard.showNotification('Error loading tracker', 'danger');
    }
};
