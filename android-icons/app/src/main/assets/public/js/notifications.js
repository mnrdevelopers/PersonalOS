...
async function loadAggregatedNotifications(filter) {
    ...
    try {
        ...
        const activeItems = allItems.filter(item => {
            const snooze = item.raw && item.raw.snoozeUntil;
            if (!snooze) return true;
            // Handle Firestore Timestamps (have .toDate()) and plain JS Dates
            const snoozeDate = typeof snooze.toDate === 'function' ? snooze.toDate() : new Date(snooze);
        ...
        if (filter === 'urgent') {
            filtered = activeItems.filter(i => i.priority === 'high' || i.status === 'overdue' || i.status === 'expired' || (i.type === 'system' && !i.raw.read));
        } else if (filter === 'tasks') {
            filtered = activeItems.filter(i => i.type === 'task');
        } else if (filter === 'expiry') {
            filtered = activeItems.filter(i => i.type === 'expiry' || i.type === 'vehicle');
        }
...
window.snoozeItem = async function(type, id, days) {
    ...
    try {
        ...
        const onclickAttr1 = activeTab ? activeTab.getAttribute('onclick') : '';
        const filterMatch1 = onclickAttr1 ? onclickAttr1.match(/'([^']+)'/) : null;
        const filter = filterMatch1 ? filterMatch1[1] : 'all';
        loadAggregatedNotifications(filter);
    } catch (e) {
...
window.clearAllReadNotifications = async function() {
    ...
    try {
        ...
        const activeTab2 = document.querySelector('#notifications-section .nav-link.active');
        const onclickAttr2 = activeTab2 ? activeTab2.getAttribute('onclick') : '';
        const filterMatch2 = onclickAttr2 ? onclickAttr2.match(/'([^']+)'/) : null;
        const filter2 = filterMatch2 ? filterMatch2[1] : 'all';
        loadAggregatedNotifications(filter2);
